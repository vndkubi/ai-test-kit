...
.github/                 GitHub Copilot adapter
  ...
  prompts/                  workflow steps 01→06 (each with an explicit I/O contract)
  agents/                   scanner / investigator / test-writer (role separation via tools)
.claude/                 Claude Code adapter
  agents/                   same 3 roles (scanner runs on a cheap model)
  commands/                 /investigate-flow /write-scenarios /fixture-backlog /build-fixtures /generate-it /onboarding-doc
...
docs/                    Accumulated knowledge (docs growing over time is GOOD — growing instructions is a tax)
  ...
  FLOW_BACKLOG.md           flow order: business-critical(×3, if applicable — money/compliance/safety;
                              e.g. zero on a product with no such flows) > churn > incidents > core entities
  FIXTURE_BACKLOG.md        single source for fixture naming + the evidence rule
  ONBOARDING.md             (generated by /onboarding-doc) derived new-dev entry point — never hand-edit
...
Week 1   PILOT the single most important flow (money transfer on a fintech product; on a product
         with no money-touching flows, churn × core-entity centrality decides instead):
...
Week 2+  Scale horizontally, 1–2 flows/week/person. Flow N+1 is cheaper (fixtures compound).
         Flow flips ✅ → 06 onboarding-doc regenerates docs/ONBOARDING.md (new-dev entry point).
Monthly  Re-run the scan in CI, diff the output → new flows/entities surface as backlog items
         + regenerate ONBOARDING.md in the same session.
```
